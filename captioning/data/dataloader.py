# 与luo完全相同, 新增注释
# 新增DataLoader/get_dataset_size(), 用于显示进度条
...
class Dataset(data.Dataset):
    ...
    def collate_func(self, batch, split):
        ...
        for sample in batch:
            ...
            att_batch.append(tmp_att) # tmp_att [196, 2048]
        ...
        # merge att_feats
        max_att_len = max([_.shape[0] for _ in att_batch]) # max_att_len 196
        data['att_feats'] = np.zeros([len(att_batch), max_att_len, att_batch[0].shape[1]], dtype = 'float32') # np.zeros(batch_size, 196, 2048)
        for i in range(len(att_batch)):
            data['att_feats'][i, :att_batch[i].shape[0]] = att_batch[i]

        data['att_masks'] = np.zeros(data['att_feats'].shape[:2], dtype='float32') # np.zeros()  data['att_feats'].shape[:2] = (batch_size, 196)
        for i in range(len(att_batch)):
            data['att_masks'][i, :att_batch[i].shape[0]] = 1 # the content is all 1?
        # set att_masks to None if attention features have same length
        if data['att_masks'].sum() == data['att_masks'].size: # data['att_masks'].sum() = batch_size * 196 = 5*196=980;  data['att_masks'].size=980
            data['att_masks'] = None
        # print('att_masks in dataloader', data['att_masks']) # att_masks in dataloader None
