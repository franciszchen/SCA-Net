    ...
    def get(self, key):
        ...
        if self.in_memory and key in self.features:
            ...
            if self.ext == '.jpg':
                ...
                # f_input = os.path.join(self.db_path, "{}-frame-{:d}.jpg".format(key, 0))
                f_input = []
                # frame_idx_list = list(range(30))
                frame_idx_list = list(range(0, 30, 2)) # 15张图像输入
                for frame_idx in frame_idx_list:
                    f_input.append(os.path.join(self.db_path, "{}-frame-{:d}.jpg".format(key, frame_idx)))
                # print("f_input:\n", f_input) # need check for training shuffle
            else:
        ...
        if isinstance(f_input, list):
            feat = []
            for f_input_frame in f_input:
                feat.append(self.loader(f_input_frame))
        else:
...
class Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.img_loader = HybridLoader(self.opt.input_img_dir, '.jpg', in_memory=self.data_in_memory) 
    ...
    def collate_func(self, batch, split): 
        ...
        ######## cz change
        att_batch_tensor = torch.stack(
            [torch.stack(att_batch[sample_idx], dim=0) for sample_idx in range(len(att_batch))],
            dim=0
        )
        # print("* att_batch_tensor:\t", type(att_batch_tensor), att_batch_tensor.shape)
        data['att_feats'] = att_batch_tensor.numpy()
        ########
        data['att_masks'] = None
        # ======================================================================== #
    ...
    def __getitem__(self, index):
        ...
        # cz print
        # need check for training shuffle
        # print("__getitem__")
        # print("ix:\t", ix)
        # print("it_pos_now:\t", it_pos_now)
        ...
        if self.use_att:
            ...
            # print("self.info['images'][ix]['file_path']:\n", self.info['images'][ix]['file_path'])
...
class DataLoader:
    def __init__(self, opt):
        ...
        print('=================================')
        print('DataLoader for video vision')
        print('=================================')
...
if __name__ == "__main__":
    ...
    print("* fc_feats:\t", type(fc_feats), fc_feats.shape) # [12, 0]
    print("* att_feats:\t", type(att_feats), att_feats.shape) # [12,5(seq_len, T),3,224,224]
    print("* labeks:\t", type(labels), labels.shape) # [12, 1, 22]
    print("* masks:\t", type(masks), masks.shape) # [12, 1, 22]
    print("* att_masks:\t", type(att_masks)) # None
