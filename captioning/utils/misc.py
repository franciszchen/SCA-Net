...
def decode_sequence(ix_to_word, seq):
    """
    cz理解
    将预测/标注的word one-hot向量, 转化成文本句子, 通过ix_to_word的vocab信息
    """
    N, D = seq.size()
    out = []
    for i in range(N):
        txt = ''
        for j in range(D):
            ix = seq[i,j] # batch内第i个样本的第j个word ix
            if ix > 0 : # 第1个word ix不能为0，不然跳出为''
                if j >= 1:
                    txt = txt + ' '
...
def model_snapshot(model, new_file, old_file=None, save_dir='./', verbose=True):
    """
    ...
    for file in os.listdir(save_dir):
        if old_file in file:
            if verbose:
                print("Removing old model  {}".format(os.path.join(expand_user(save_dir), file)))
            os.remove(os.path.join(expand_user(save_dir), file)) 
    if verbose:
        print("Saving new model to {}".format(os.path.join(expand_user(save_dir), new_file)))
    torch.save(model.state_dict(), os.path.join(expand_user(save_dir), new_file))
...
class ReduceLROnPlateau(torch.optim.Optimizer):
    ...
    def __init__(self, optimizer, mode='min', factor=0.1, patience=10, verbose=False, threshold=0.0001, threshold_mode='rel', cooldown=0, min_lr=0, eps=1e-08):
        print("* mode: {}, factor: {}, patience: {}, verbose: {}, threshold: {}, threshold_mode: {}".format(
            mode, factor, patience, verbose, threshold, threshold_mode
        ))
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer=optimizer, mode=mode, factor=factor, patience=patience, verbose=verbose, 
            threshold=threshold, threshold_mode=threshold_mode, cooldown=cooldown, min_lr=min_lr, eps=eps
        ) # xu 搞错了参数verbose的顺序，补全形参
        self.optimizer = optimizer
...
def get_std_opt(model, optim_func='adam', factor=1, warmup=2000):
    ...
    return NoamOpt(
        ...
        factor=factor, 
        warmup=warmup,
        optimizer=optim_func(model.parameters(), lr=3e-4, betas=(0.9, 0.98), eps=1e-9)
    )


   # For the XE training, we initialize the learning rate to 3e-4 and follow the learning rate scheduling strategy with 20000 warm-up steps