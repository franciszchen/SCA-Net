...
def getCOCO(dataset):
    if 'coco' in dataset:
        ...
        print('mengya: annFile in eval_utilis ！！！！！！！！！！！！！！！！！！:', annFile)
    # cz add
    elif 'EndoVis18' in dataset:
        annFile = 'data/EndoVis18/EndoVis18_afterprepro_val.json'
        print('annFile in eval_utilis:', annFile)
    # elif 'wales_case5' in dataset:
    #     annFile = 'data/wales_case5/caption-case5-0128_afterprepro_val.json'
    #     print('annFile in eval_utilis:', annFile)
    elif 'wales_case41' in dataset:
        annFile = 'data/wales_case41/caption-case41-30s-0201_afterprepro_val.json'
        print('annFile in eval_utilis:', annFile)
    
    elif 'wales_case89' in dataset:
        annFile = 'data/wales_case89/caption-case89-30s-0323_afterprepro_val.json'
        print('annFile in eval_utilis:', annFile)
...
def language_eval(dataset, preds, preds_n, eval_kwargs, split):
    ...
    if len(preds_n) > 0:
        ...
        print(" * cz: dataset_file: {}".format(dataset_file)) # data/wales_case89/caption-case89-30s-0323_afterprepro.json
    ...
    print(" * cz: dataset: {}".format(dataset))
    ...
    print('* /language_eval/ using %d/%d predictions' % (len(preds_filt), len(preds)))
...
def eval_split(
    ...
    verbose_beam = eval_kwargs.get('verbose_beam', 0) # 0 
    verbose_loss = eval_kwargs.get('verbose_loss', 1) # 1
    num_images = eval_kwargs.get('num_images', eval_kwargs.get('val_images_use', -1))
    split = eval_kwargs.get('split', 'val') # my: split: 'val' # val
    lang_eval = eval_kwargs.get('language_eval', 0) # 1

    dataset = eval_kwargs.get('dataset', 'coco') # my:字典get()函数返回指定键的值，如果值不在字典中返回默认值 # data/EndoVis18/EndoVis18_afterprepro.json
    # dataset = eval_kwargs.get('dataset', 'miccai18') # my:字典get()函数返回指定键的值，如果值不在字典中返回默认值

    beam_size = eval_kwargs.get('beam_size', 1) # 1
    sample_n = eval_kwargs.get('sample_n', 1) # 1
    remove_bad_endings = eval_kwargs.get('remove_bad_endings', 0)
    os.environ["REMOVE_BAD_ENDINGS"] = str(remove_bad_endings) # Use this nasty way to make other code clean since it's a global configuration
    device = eval_kwargs.get('device', 'cuda')

    print('---')
    print("* eval_split info")
    print("verbose_beam: ", verbose_beam) # 0
    print("verbose_loss: ", verbose_loss) # 1
    print("num_images: ", num_images) # 447
    print("val_images_use", eval_kwargs.get('val_images_use', -1)) # 447
    print("split: ", split)     # val
    print("lang_eval: ", lang_eval) # 1
    print("dataset: ", dataset) # data/EndoVis18/EndoVis18_afterprepro.json
    print("beam_size: ", beam_size) # 1
    ...
    while True:
        ...
        if labels is not None and verbose_loss:
            ...
            with torch.no_grad():
                # 调用_forward()
                output, (_, _, _), (_, _), (_, _) = model(
                    # prototypes, 
                    prototypes_text, 
                    prototypes_visual,
                    fc_feats, att_feats, labels[..., :-1], att_masks)
                loss = crit(output, labels[..., 1:], masks[..., 1:]).item()
                # loss = crit(model(fc_feats, att_feats, labels[..., :-1], att_masks), labels[..., 1:], masks[..., 1:]).item()
            loss_sum = loss_sum + loss
            loss_evals = loss_evals + 1
            # ------------------------------------------------- #
        """
        注意此函数中, seq为模型预测, 后面又被封装成'caption': sent,
        labels为caption ground-truth
        """
        ...
        # cz add
        # print("seq:\t", type(seq), seq.shape) # torch.Size([9, 20])
        # print("labels:\t", type(labels), labels.shape) # torch.Size([9, 1, 18])
        sents_label = utils.decode_sequence(model.vocab, labels.squeeze(dim=1)[:, 1:])
        ...
        for k, (sent, sent_label) in enumerate(zip(sents, sents_label)):
            ############ my: prediction results #################
            # original one
            # entry = {'image_id': data['infos'][k]['id'], 'caption': sent, 'perplexity': perplexity[k].item(), 'entropy': entropy[k].item()}
            data['infos'][k]['id'] = data['infos'][k]['ix']
            entry = {
                'image_id': data['infos'][k]['ix'], 
                'caption': sent, 
                'gt_label': sent_label, # cz add
...
def eval_split_n(model, n_predictions, input_data, eval_kwargs={}):
    ...
    print("eval_split_n / sample_n_method: ", sample_n_method)
    tmp_eval_kwargs = eval_kwargs.copy()
    if sample_n_method == 'bs':
        print("eval_split_n #1")
